import { buildContributionSummary, buildMissingMonthsWithRange, buildValidation } from "../pwa/js/report/report_calculations.js";
...
function assertIncludes(collection, value, label) {
  if (!collection.includes(value)) {
    throw new Error(`${label} expected ${value}, got ${JSON.stringify(collection)}`);
  }
}

function buildRecord({ nestEE, nestER, natInsNumber, taxCode, grossPay, netPay }) {
  return {
...
function runContributionSummaryTest() {
  const entries = [
    {
      record: buildRecord({ nestEE: 50, nestER: 30, grossPay: 100, netPay: 100 }),
    ...
    {
      record: buildRecord({ nestEE: 60, nestER: 40, grossPay: 100, netPay: 100 }),
...
function runValidationTest() {
  const entry = {
    record: buildRecord({ nestEE: 0, nestER: 0, grossPay: 100, netPay: 90 }),
