...
function selectPaymentsBand(bands) {
  ...
  bands.forEach((band, index) => {
    ...
    const numericCount = lines.reduce((total, line) => total + countNumericTokens(line), 0);
...
function computeColumnCentroids(lineItems, columnCount) {
  ...
  let centroids = Array.from({ length: columnCount }, (_, index) => minX + step * index);
  ...
  for (let iteration = 0; iteration < 4; iteration += 1) {
    ...
    centroids = centroids.map((centroid, index) => {
      ...
      const sum = buckets[index].reduce((total, value) => total + value, 0);
...
function computeCentroidsFromValues(values, columnCount) {
  ...
  let centroids = Array.from({ length: columnCount }, (_, index) => minValue + step * index);
...
function bucketLinesByColumn(lineItems, columnCount, splitX) {
  ...
  const useSplit =
    columnCount === 2 && typeof splitX === "number" && Number.isFinite(splitX);
  ...
  lineItems.forEach((line) => {
    ...
    line.items.forEach((item) => {
      const columnIndex = useSplit ? (item.x <= splitX ? 0 : 1) : nearestIndexForX(item.x);
...
function extractAddressFromLines(lines, employeeName) {
  ...
  const filtered = trimmed.filter(
    (line, index) => !(index === 0 && employeeName && line.startsWith(employeeName))
  );
  return {
...
function parsePaymentsFromLines(lines) {
  const result = {
    hourly: {
      basic: { title: "Basic Hours", units: null, rate: null, amount: null },
...
function splitLineItemsByGlobalSplit(lineItems) {
  ...
  const splitX = deductionSplitX !== null ? deductionSplitX - 2 : (centroids[1] + centroids[2]) / 2;
...
function extractMiscLineItems(lines, positionalLineItems) {
  ...
  if (positionalLines.length) {
    ...
    if (columns[0].length || columns[1].length) {
      parseMiscLines(columns[0], "payments", miscPayments, miscDeductions, basicOverrides);
      parseMiscLines(columns[1], "deductions", miscPayments, miscDeductions, basicOverrides);
  ...
  lines.forEach((line) => {
    ...
    parseMiscLines([line], mode, miscPayments, miscDeductions, basicOverrides);
  });
...
function buildPayrollDocument({ text, lines, lineItems }) {
  ...
  if (positionalLines.length) {
    ...
    const paymentsBandIndex = selectPaymentsBand(bands);
    const paymentsBand = bands[paymentsBandIndex] || [];
    const { leftLines, rightLines } = splitLineItemsByGlobalSplit(paymentsBand);
  ...
  const nationalInsurance = parseNumericValue(extractField(text, PATTERNS.nationalInsurance));
  ...
  const nationalInsuranceTD = parseNumericValue(extractField(text, PATTERNS.nationalInsuranceTD));
  const employeePensionTD = parseNumericValue(extractField(text, PATTERNS.employeePensionTD));
  const employerPensionTD = parseNumericValue(extractField(text, PATTERNS.employerPensionTD));
