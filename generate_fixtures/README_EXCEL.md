...
> **Note — no `labels.json` pattern here (yet):** The PDF fixture pipeline uses a `labels.json` file as a single source of truth for label strings shared between the generator and the PWA parser, with a codegen step to derive regex patterns from those strings. The pension contribution side does not follow this pattern yet because different providers may not produce Excel files at all — a future provider could deliver a CSV, a PDF, or a proprietary format. Until a second provider exists and the shape of its output is known, it is premature to define a shared label schema. If and when a second provider is added, the contribution parser (`pwa/js/parse/contribution_validation.js`) should be refactored into per-provider parser modules (mirroring the PDF parser structure under `pwa/js/parse/formats/<provider>/`), at which point `labels.json` becomes the right pattern to adopt here too.
